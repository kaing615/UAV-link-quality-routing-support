...
<p align="center">
  <i>Application of Graph Neural Networks for Link Quality Prediction and Routing Support in UAV Networks</i>
</p>
...
6. **Huấn luyện mô hình GNN cho bài toán dự đoán liên kết:**
   Sử dụng **Edge-Aware GraphSAGE** — biến thể GraphSAGE do nhóm đề xuất — làm mô hình chính. Khác với GraphSAGE gốc chỉ tổng hợp đặc trưng nút từ hàng xóm, Edge-Aware GraphSAGE đưa cả **đặc trưng cạnh** (RSSI, SNR, khoảng cách...) vào bước message passing, giúp embedding của mỗi UAV phản ánh trực tiếp chất lượng các liên kết vô tuyến xung quanh nó.

7. **So sánh với các baseline và mô hình đối chiếu:**
   Đối chiếu hiệu quả với các phương pháp học máy truyền thống (**Logistic Regression**, **Random Forest**, **XGBoost**, **MLP**), phương pháp heuristic dựa trên ngưỡng RSSI/SNR, và các kiến trúc GNN khác (**GraphSAGE gốc**, **GAT**).
...
9. **Đánh giá toàn hệ thống ở hai mức:**
   - **Mức mô hình học máy:** Accuracy, Precision, Recall, F1-score, ROC-AUC, PR-AUC, thời gian suy luận
   - **Mức hiệu năng mạng:** Route Lifetime, Route Changes, Packet Delivery Ratio, End-to-End Delay — đo bằng **replay** trên test snapshot, so sánh 4 chiến lược chọn tuyến (shortest-hop ≈ OLSR, delay-weighted, prediction-assisted) và khảo sát ngưỡng lọc link `p_th` (chi tiết: [src/routing/README.md](./src/routing/README.md))

10. **Đánh giá theo hai giao thức và phân tích ablation:**
   - **Within-run:** train/val/test là các cửa sổ thời gian khác nhau của cùng một kịch bản mô phỏng
   - **Cross-run (Leave-One-Run-Out):** huấn luyện trên N−1 kịch bản, kiểm thử trên toàn bộ kịch bản còn lại chưa từng thấy — đo khả năng tổng quát hóa sang topology/mobility mới
   - **Ablation đặc trưng cạnh:** huấn luyện lại các GNN không dùng edge features để định lượng vai trò của đặc trưng liên kết so với cấu trúc đồ thị
...
| ---------------------------------- | -------------------------------------------------------------------------- |
| **Ngôn ngữ chính**                 | Python (pipeline), C++ (kịch bản ns-3)                                      |
| **Mô phỏng mạng UAV**              | **ns-3** (802.11 ad-hoc + `ns3::olsr` thật, Nakagami fading; mobility: Random Waypoint, Gauss-Markov) — hướng chính; Python simulator công thức hóa cho lặp nhanh |
| **Xử lý dữ liệu**                  | NumPy, Pandas                                                               |
| **Routing support**                | NetworkX (Dijkstra trên trọng số `w = 1 − ŷ`, đánh giá replay)              |
| **Trực quan hóa**                  | Matplotlib                                                                  |
| **Framework học sâu**              | PyTorch                                                                     |
| **Biểu diễn và huấn luyện đồ thị** | PyTorch Geometric (SAGEConv, GATConv, custom MessagePassing)                |
