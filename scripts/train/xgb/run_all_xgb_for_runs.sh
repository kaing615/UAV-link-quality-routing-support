...
XGB_MODULE="src.training.baselines.xgb_baseline"
RUN_PATTERN="${1:-*}"

if [[ ! -f "${PROJECT_ROOT}/src/training/baselines/xgb_baseline.py" ]]; then
  echo "XGBoost baseline script not found: ${PROJECT_ROOT}/src/training/baselines/xgb_baseline.py"
  exit 1
fi
...
for run_dir in "${matching_runs[@]}"; do
  ...
  if "${PYTHON_BIN}" -m "${XGB_MODULE}" --run-name "${run_name}"; then
    echo "[OK] XGBoost completed for ${run_name}"
  else
    echo "[FAIL] XGBoost failed for ${run_name}"
    failures=$((failures + 1))
  fi
done
