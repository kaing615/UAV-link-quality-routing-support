...
metadata: 
  node_type: memory
  type: project
  originSessionId: 636e836f-da02-461b-8a01-f2a78d83ee13
---
...
- **Kết quả routing 100 run** (outputs/aggregates/routing/):
  - gnn/xgb-assisted vs shortest-hop: lifetime +41% (0.50 vs 0.35),
    PDR@t+1 +44% (0.144 vs 0.100), giá = +0.4 hop (~+0.8ms delay)
  - gnn ≈ xgb (chênh <1.5%) — giá trị ở tín hiệu dự đoán, không phân thắng kiến trúc
- **OLSR thật đã so sánh**: strategy "olsr" đọc từ traffic_log.csv (route_path ns3::olsr thật),
  1 cặp src-dst mỗi run. results: olsr route_found_rate=0.66, lifetime=0.20 vs gnn=0.50
  (vì OLSR HELLO/TC delay làm topology OLSR "stale" so với ground-truth).
  Chart: routing_comparison_olsr_pair.png
- p_th sweep (0.1-0.7, --strict): lifetime 0.50→2.82, PDR 0.14→0.77, changes
  3.16→0.99 nhưng route_found_rate 100%→16.5%; sweet spot ~0.3
  → outputs/aggregates/routing/{pth_sweep.csv, pth_tradeoff.png}
- **CHƯA làm: Horizon sweep** (H=1,3,5,10) — xem prediction-assisted có cải thiện
  ở horizon xa hơn không (nơi GNN có thể vượt tabular vì thấy topology mới)
...
- src/routing/README.md: Thêm OLSR comparison results, key findings, p_th sweep results
- docs/experiment_plan.md: Viết lại toàn bộ với đầy đủ results từ 100 runs
- docs/quick_start.md: Thêm horizon sweep section
- scripts/routing/sweep_horizon.sh: Tạo script cho horizon sweep experiment

## Remaining steps
- **CHẠY HORIZON SWEEP**: ./scripts/routing/sweep_horizon.sh 'ns3big_*' edge-sage '1,3,5,10'
  - Xem GNN có vượt tabular ở horizon xa hơn không
  - Output: horizon_sweep.csv + horizon_tradeoff.png
- Soạn phần "Phân tích kết quả" hoàn chỉnh cho báo cáo (đã có dàn ý trong docs/experiment_plan.md)
