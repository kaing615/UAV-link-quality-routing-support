...
def train_one_epoch(model: nn.Module, loader, optimizer: torch.optim.Optimizer, criterion: nn.Module, device: torch.device) -> float:
    model.train()
    total_loss = 0.0
    for batch in loader:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--no-tune-threshold', action='store_true', default=False, help='Disable per-run decision threshold tuning on the val split (default: tuning ON)')
    p.add_argument('--no-edge-features', action='store_true', default=False, help="Ablation: drop edge features from message passing and decoder (model_id gets '-noedge' suffix)")
...
def main() -> None:
    ...
    model = model_cls(node_in_channels=NODE_IN, edge_in_channels=EDGE_IN, hidden_channels=args.hidden, num_layers=args.num_layers, dropout=args.dropout, use_edge_features=not args.no_edge_features).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = None
    if args.lr_scheduler:
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=10, min_lr=1e-05)
    with Live(dir=str(output_dir / 'dvclive'), report='auto') as live:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                print(f'  ep {epoch:3d} | loss={train_loss:.4f} | val_macro_f1={val_f1:.4f} | best={best_macro_f1:.4f} @ ep{best_epoch} | lr={current_lr:.2e}')
    ...
    metadata = {'model_id': model_id, 'model_name': model_name, 'run_name': args.run_name, 'hidden_channels': args.hidden, 'num_layers': args.num_layers, 'dropout': args.dropout, 'lr': args.lr, 'weight_decay': args.weight_decay, 'lr_scheduler': args.lr_scheduler, 'use_edge_features': not args.no_edge_features, 'threshold': threshold, 'best_epoch': best_epoch, 'best_val_macro_f1': best_macro_f1}
    (output_dir / 'metadata.json').write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    print(f"[OK]  val : macro_f1={val_metrics['macro_f1']:.4f}  f1={val_metrics['f1']:.4f}  recall={val_metrics['recall']:.4f}")
    print(f"[OK]  test: macro_f1={test_metrics['macro_f1']:.4f}  f1={test_metrics['f1']:.4f}  recall={test_metrics['recall']:.4f}")
