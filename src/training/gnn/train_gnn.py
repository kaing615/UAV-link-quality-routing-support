...
def train_one_epoch(
    model: nn.Module, loader, optimizer: torch.optim.Optimizer, criterion: nn.Module, device: torch.device
) -> float:
    model.train()
    total_loss = 0.0
    for batch in loader:
...
def main() -> None:
    ...
    with Live(dir=str(output_dir / "dvclive"), report="auto") as live:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                print(
                    f"  ep {epoch:3d} | loss={train_loss:.4f} | val_macro_f1={val_f1:.4f} | best={best_macro_f1:.4f} @ ep{best_epoch} | lr={current_lr:.2e}"
                )
    ...
    print(
        f"[OK]  val : macro_f1={val_metrics['macro_f1']:.4f}  f1={val_metrics['f1']:.4f}  recall={val_metrics['recall']:.4f}"
    )
    print(
        f"[OK]  test: macro_f1={test_metrics['macro_f1']:.4f}  f1={test_metrics['f1']:.4f}  recall={test_metrics['recall']:.4f}"
    )
