...
def main() -> None:
    ...
    model = model_cls(node_in_channels=NODE_IN, edge_in_channels=EDGE_IN, hidden_channels=args.hidden, num_layers=args.num_layers, dropout=args.dropout).to(device)
    ...
    with Live(dir=str(output_dir / 'dvclive'), report='auto') as live:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 10 == 0 or epoch == 1:
                print(f'  ep {epoch:3d} | loss={train_loss:.4f} | val_macro_f1={val_f1:.4f} | best={best_macro_f1:.4f} @ ep{best_epoch}')
    ...
    metadata = {'model_id': model_id, 'model_name': model_name, 'protocol': 'leave-one-run-out', 'test_run': args.test_run, 'train_runs': train_runs, 'hidden_channels': args.hidden, 'num_layers': args.num_layers, 'dropout': args.dropout, 'lr': args.lr, 'weight_decay': args.weight_decay, 'lr_scheduler': args.lr_scheduler, 'threshold': threshold, 'best_epoch': best_epoch, 'best_val_macro_f1': best_macro_f1}
