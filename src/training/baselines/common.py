...
def find_best_threshold(model, val_df: pd.DataFrame, lo: float=0.3, hi: float=0.7, min_gain: float=0.02) -> tuple[float, float]:
...
def evaluate_split(model, model_id: str, model_name: str, df: pd.DataFrame, split_name: str, threshold: float | None=None) -> tuple[dict[str, object], pd.DataFrame]:
...
def save_outputs(output_dir: Path, model, metadata: dict[str, object], metrics_rows: list[dict[str, object]], prediction_frames: dict[str, pd.DataFrame]) -> None:
