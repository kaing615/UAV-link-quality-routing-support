...
def build_xgb(random_state: int) -> XGBClassifier:
    return XGBClassifier(n_estimators=300, max_depth=6, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8, objective='binary:logistic', eval_metric='logloss', random_state=random_state)
...
def main() -> None:
    ...
    metadata = {'model_id': MODEL_ID, 'model_name': MODEL_NAME, 'train_strategy': train_strategy, 'threshold': threshold, 'feature_columns': FEATURE_COLUMNS, 'n_estimators': int(model.n_estimators), 'max_depth': int(model.max_depth), 'learning_rate': float(model.learning_rate), 'subsample': float(model.subsample), 'colsample_bytree': float(model.colsample_bytree)}
