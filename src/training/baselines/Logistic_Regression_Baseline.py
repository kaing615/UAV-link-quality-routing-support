...
def build_logreg(random_state: int) -> LogisticRegression:
    return LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=1000, class_weight=None, random_state=random_state)
...
def main() -> None:
    ...
    model, train_strategy = fit_logreg(weighted_train_df, oversampled_train_df, args.random_state)
    ...
    metadata = {'model_id': MODEL_ID, 'model_name': MODEL_NAME, 'train_strategy': train_strategy, 'feature_columns': FEATURE_COLUMNS, 'penalty': model.penalty, 'C': float(model.C), 'solver': model.solver, 'max_iter': int(model.max_iter)}
