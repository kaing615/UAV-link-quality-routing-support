...
def build_rf(random_state: int) -> RandomForestClassifier:
    return RandomForestClassifier(n_estimators=200, max_depth=None, min_samples_split=2, min_samples_leaf=1, max_features='sqrt', n_jobs=-1, random_state=random_state)
...
def main() -> None:
    ...
    model, train_strategy = fit_rf(weighted_train_df, oversampled_train_df, args.random_state)
    ...
    metadata = {'model_id': MODEL_ID, 'model_name': MODEL_NAME, 'train_strategy': train_strategy, 'feature_columns': FEATURE_COLUMNS, 'n_estimators': int(model.n_estimators), 'max_depth': model.max_depth, 'max_features': model.max_features}
