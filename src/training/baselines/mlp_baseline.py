...
def build_mlp(random_state: int) -> MLPClassifier:
    return MLPClassifier(hidden_layer_sizes=(32, 16), activation='relu', solver='adam', alpha=0.0001, learning_rate_init=0.001, max_iter=500, early_stopping=True, validation_fraction=0.1, n_iter_no_change=20, random_state=random_state)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--run-name', type=str, default=None, help='Preprocessed run name under data/graph_dataset/<RUN_NAME>/baseline_standardized. If provided, default input/output paths are resolved from that run.')
...
def main() -> None:
    ...
    metadata = {'model_id': MODEL_ID, 'model_name': MODEL_NAME, 'train_strategy': train_strategy, 'threshold': threshold, 'feature_columns': FEATURE_COLUMNS, 'n_iter': int(model.n_iter_), 'loss_curve_length': len(model.loss_curve_), 'best_validation_score': float(model.best_validation_score_) if getattr(model, 'best_validation_score_', None) is not None else None}
