...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--summary-csv', type=Path, default=Path('outputs/aggregates/all_models/summary_by_model_split.csv'), help='Path to summary_by_model_split.csv')
    parser.add_argument('--output-dir', type=Path, default=Path('outputs/aggregates/all_models'), help='Directory to save the plots.')
    parser.add_argument('--title', type=str, default='Performance Comparison of All Models (Test Split)', help='Chart title.')
...
def main() -> None:
    ...
    if test_df.empty:
        ...
        return
    order = ['threshold', 'logreg', 'rf', 'xgb', 'mlp', 'gat', 'graphsage', 'edge-sage', 'gat-noedge', 'graphsage-noedge', 'edge-sage-noedge']
    ...
    for idx, order_id in enumerate(present):
        ...
        ax.bar(positions, means, width, label=MODEL_NAMES.get(order_id, order_id), color=COLORS.get(order_id, '#7f7f7f'), edgecolor='#ffffff', linewidth=1, yerr=stds, error_kw={'ecolor': '#555555', 'capsize': 3, 'elinewidth': 0.8})
        for pos, val in zip(positions, means):
            ax.text(pos, val + 0.015, f'{val:.2f}', ha='center', va='bottom', fontsize=8, color='#333333', fontweight='semibold')
    ...
    ax.set_ylim(0, 1.1)
    ax.legend(loc='lower center', bbox_to_anchor=(0.5, 1.02), ncol=3, frameon=True, facecolor='#ffffff', edgecolor='#dddddd', fontsize=9)
