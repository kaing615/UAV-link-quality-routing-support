...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--outputs-root', type=Path, default=Path('outputs/baselines'), help='Root directory containing outputs/baselines/<MODEL_ID>/<RUN_NAME>/metrics.csv')
    parser.add_argument('--model-pattern', type=str, default='*', help='Glob pattern for model_id directories under outputs-root.')
    parser.add_argument('--run-pattern', type=str, default='*', help='Glob pattern for run_name directories under each model directory.')
    parser.add_argument('--output-dir', type=Path, default=Path('outputs/aggregates/baselines'), help='Directory to save aggregated CSV files.')
...
def collect_detail_rows(outputs_root: Path, model_pattern: str, run_pattern: str) -> pd.DataFrame:
    ...
    for model_dir in sorted(path for path in outputs_root.iterdir() if path.is_dir()):
        ...
        for run_dir in sorted(path for path in model_dir.iterdir() if path.is_dir()):
            ...
            frames.append(df)
    if not frames:
        raise FileNotFoundError(f"No metrics.csv found under {outputs_root} for model_pattern='{model_pattern}' and run_pattern='{run_pattern}'")
