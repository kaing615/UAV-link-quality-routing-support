...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-pattern", type=str, default="*", help="Glob pattern for model_id directories under outputs-root."
    )
    parser.add_argument(
        "--run-pattern", type=str, default="*", help="Glob pattern for run_name directories under each model directory."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def collect_detail_rows(outputs_root: Path, model_pattern: str, run_pattern: str) -> pd.DataFrame:
    ...
    if not frames:
        raise FileNotFoundError(
            f"No metrics.csv found under {outputs_root} for model_pattern='{model_pattern}' and run_pattern='{run_pattern}'"
        )
