"""Recompute metrics.csv for already-trained GNN runs without retraining.

Loads best_model.pt + metadata.json for every (model, run) under outputs/gnn,
re-evaluates val/test with the current evaluate_split (which now reports
ROC-AUC, PR-AUC and inference time), and rewrites metrics.csv in place.
The tuned decision threshold stored in metadata.json is reused unchanged.
"""
