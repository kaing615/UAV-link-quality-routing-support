    ...
    for col_idx, (feat_name, vmin, vmax) in ranges.items():
        ...
        if below > 0 or above > 0:
            issues.append({
                ...
                "feature": feat_name,
                "message": f"{feat_name}: {below} below {vmin}, {above} above {vmax}",
...
def check_class_balance(labels: torch.Tensor, run_name: str) -> list[dict]:
    ...
    if ratio > 0.95 or ratio < 0.05:
        issues.append({
            ...
            "run_name": run_name,
            "message": f"Extreme class imbalance: positive_ratio={ratio:.4f} ({n_pos}/{n_total})",
...
def main() -> None:
    ...
    run_dirs = sorted(
        ...
        if d.is_dir() and (d / "graph_dataset").exists()
    ) if args.data_dir.exists() else []
    ...
    print(f"[DATA QUALITY] {report['summary']['status']}: "
