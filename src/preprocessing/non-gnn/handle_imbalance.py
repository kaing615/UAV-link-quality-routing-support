...
def oversample_minority_class(df: pd.DataFrame, label_column: str, target_ratio: float, random_state: int) -> pd.DataFrame:
    if target_ratio <= 0:
    ...
    sampled_minority = minority_df.sample(n=extra_needed, replace=True, random_state=random_state)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--run-name', type=str, default=None, help='Preprocessed run name under data/graph_dataset/<RUN_NAME>/baseline_standardized. If provided, default train/output paths are resolved from that run.')
    parser.add_argument('--train-csv', type=Path, default=DEFAULT_TRAIN_CSV, help='Path to the standardized train CSV.')
    parser.add_argument('--output-dir', type=Path, default=DEFAULT_OUTPUT_DIR, help='Directory to save imbalance-handling outputs.')
    parser.add_argument('--label-column', type=str, default='label', help='Binary label column name.')
    parser.add_argument('--method', choices=['weights', 'oversample', 'both'], default='both', help='Imbalance handling strategy to export.')
    parser.add_argument('--target-ratio', type=float, default=1.0, help='Minority/majority target ratio after oversampling. 1.0 means fully balanced.')
    parser.add_argument('--random-state', type=int, default=42, help='Random seed used for oversampling and shuffle.')
...
def main() -> None:
    ...
    summary = {'input_csv': str(train_csv), 'label_column': args.label_column, 'method': args.method, 'target_ratio': args.target_ratio, 'random_state': args.random_state, 'original_class_counts': class_counts(train_df, args.label_column)}
    if args.method in {'weights', 'both'}:
        ...
        summary['weight_map'] = {str(label): float(weighted_df.loc[weighted_df[args.label_column] == label, 'sample_weight'].iloc[0]) for label in sorted(weighted_df[args.label_column].unique().tolist())}
    if args.method in {'oversample', 'both'}:
        oversampled_df = oversample_minority_class(df=train_df, label_column=args.label_column, target_ratio=args.target_ratio, random_state=args.random_state)
