...
def oversample_minority_class(
    df: pd.DataFrame, label_column: str, target_ratio: float, random_state: int
) -> pd.DataFrame:
    if target_ratio <= 0:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Preprocessed run name under data/graph_dataset/<RUN_NAME>/baseline_standardized. If provided, default train/output paths are resolved from that run.",
    )
    parser.add_argument("--train-csv", type=Path, default=DEFAULT_TRAIN_CSV, help="Path to the standardized train CSV.")
    parser.add_argument(
        "--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR, help="Directory to save imbalance-handling outputs."
    )
...
def main() -> None:
    ...
    if args.method in {"oversample", "both"}:
        oversampled_df = oversample_minority_class(
            df=train_df, label_column=args.label_column, target_ratio=args.target_ratio, random_state=args.random_state
        )
