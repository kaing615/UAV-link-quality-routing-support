...
def scaler_stats_dict(scaler: StandardScaler) -> dict[str, dict[str, float]]:
    return {
        feature: {"mean": float(mean), "scale": float(scale)}
...
def save_outputs(output_dir: Path, split_frames: dict[str, pd.DataFrame], scaler: StandardScaler) -> dict[str, Path]:
    ...
    scaler_stats_path.write_text(
        json.dumps(
            {"fit_on_split": "train", "feature_columns": FEATURE_COLUMNS, "stats": scaler_stats_dict(scaler)}, indent=2
        ),
        encoding="utf-8",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Preprocessed run name under data/graph_dataset/<RUN_NAME>. If provided, default edges/splits/output paths are resolved from that run.",
    )
    parser.add_argument("--edges-labeled", type=Path, default=DEFAULT_EDGES_LABELED, help="Path to labeled edge table.")
    parser.add_argument(
        "--splits", type=Path, default=DEFAULT_SPLITS, help="Path to time-based train/val/test split file."
    )
    parser.add_argument(
        "--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR, help="Directory to save standardized CSV files."
    )
