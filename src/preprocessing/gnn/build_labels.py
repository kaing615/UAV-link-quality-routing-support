...
def build_labeled_edges(edges_features_csv: Path, output_csv: Path, tau_snr: float=18.0, tau_loss: float=0.1, tau_delay: float=10.0) -> Path:
    ...
    next_shifted = next_shifted.rename(columns={'connected': 'connected_next', 'snr': 'snr_next', 'packet_loss': 'packet_loss_next', 'delay': 'delay_next'})
    ...
    merged['label'] = [assign_label(cn, sn, pl, de, tau_snr, tau_loss, tau_delay) for cn, sn, pl, de in zip(merged['connected_next'], merged['snr_next'], merged['packet_loss_next'], merged['delay_next'])]
...
if __name__ == '__main__':
    args = parse_args()
    out = build_labeled_edges(args.edges_features, args.output, tau_snr=args.tau_snr, tau_loss=args.tau_loss, tau_delay=args.tau_delay)
