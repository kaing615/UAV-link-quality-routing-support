...
def build_graph_records(nodes_csv: Path, edges_labeled_csv: Path, split_csv: Path, output_dir: Path) -> Path:
    ...
    for time in sorted(edges['time'].unique().tolist()):
        ...
        graph = {'time': int(time), 'split': split, 'node_ids': node_ids, 'x': x, 'edge_index': edge_index, 'edge_attr': edge_attr, 'edge_label_index': edge_label_index, 'edge_label': edge_label, 'edge_label_name': edge_label_name, 'node_feature_names': NODE_FEATURES, 'edge_feature_names': EDGE_FEATURES}
