...
def build_time_split(
    edges_labeled_csv: Path, output_dir: Path, train_ratio: float = 0.7, val_ratio: float = 0.15
) -> Path:
    ...
    split_df["split"] = split_df["time"].map(lambda t: "train" if t in train_set else "val" if t in val_set else "test")
