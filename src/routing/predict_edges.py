"""Run a trained GNN edge classifier over the test snapshots of a run and
export per-edge stability scores keyed by (time, src, dst).

The training pipeline saves test_predictions.csv without edge identity, so
the routing module re-runs inference from best_model.pt + test.pt, where each
stored graph still carries `time` and `node_ids`.

Output: outputs/routing/<RUN_NAME>/edge_predictions_<MODEL_ID>.csv
"""
