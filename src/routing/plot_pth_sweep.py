...
REF_COLOR = '#9e9e9e'
PANELS = [('route_found_rate', 'Route Found Rate', 'Khả năng duy trì liên thông'), ('mean_route_lifetime', 'Route Lifetime (steps)', 'Độ an toàn của tuyến'), ('mean_realized_pdr_t1', 'Realized PDR @ t+1', 'Độ an toàn của tuyến'), ('mean_route_changes', 'Route Changes / session', 'Chi phí duy trì tuyến')]
...
def plot(df: pd.DataFrame, output_dir: Path, title: str) -> Path:
    ...
    agg = pred.groupby(['strategy', 'p_th']).agg(**{f'{col}_{stat}': (col, stat) for col, _, _ in PANELS for stat in ['mean', 'std']}).reset_index()
    ...
    for ax, (col, label, aspect) in zip(axes, PANELS):
        for st in [s for s in ['xgb', 'gnn'] if s in pred['strategy'].unique()]:
            ...
            ax.errorbar(sub['p_th'], sub[f'{col}_mean'], yerr=sub[f'{col}_std'], marker='o', capsize=3, label=STRATEGY_LABELS[st], color=COLORS[st])
        if not hop.empty and col in hop.columns:
            ax.axhline(float(hop[col].mean()), color=REF_COLOR, linestyle='--', linewidth=1.2, label='Shortest Hop (ref)')
