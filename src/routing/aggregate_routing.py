...
PANELS = [('mean_route_lifetime', 'Route Lifetime (steps)', False), ('mean_route_changes', 'Route Changes / session', True), ('mean_realized_pdr_t1', 'Realized PDR @ t+1', False), ('mean_e2e_delay_ms', 'E2E Delay (ms)', True)]

def aggregate(routing_root: Path, output_dir: Path, pattern: str='*', filename: str='summary.csv', out_suffix: str='') -> pd.DataFrame:
    ...
    grouped = detailed.groupby('strategy').agg(n_runs=('run_name', 'nunique'), n_sessions=('n_sessions', 'sum'), **{f'{col}_{stat}': (col, stat) for col in ['route_found_rate', 'mean_hops', 'mean_e2e_delay_ms', 'mean_est_pdr', 'mean_route_lifetime', 'survival_at_1', 'mean_realized_pdr_t1', 'mean_route_changes', 'disconnected_rate'] for stat in ['mean', 'std']}).reset_index()
...
def plot(grouped: pd.DataFrame, output_dir: Path, title: str, filename: str='routing_comparison.png') -> Path:
...
if __name__ == '__main__':
    args = parse_args()
    cols = ['strategy', 'n_runs', 'mean_route_lifetime_mean', 'mean_route_changes_mean', 'mean_realized_pdr_t1_mean', 'mean_e2e_delay_ms_mean']
    ...
    try:
        grouped_olsr = aggregate(args.routing_root, args.output_dir, pattern=args.pattern, filename='summary_olsr_pair.csv', out_suffix='_olsr_pair')
        out_png_olsr = plot(grouped_olsr, args.output_dir, 'Routing Strategy Comparison — Same Pair as OLSR Recorded Flow', filename='routing_comparison_olsr_pair.png')
