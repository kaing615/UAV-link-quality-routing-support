...
def load_model() -> None:
    ...
    if not weights_path.exists():
        raise RuntimeError(
            f"Model weights not found at {weights_path}. Set MODEL_DIR to a directory containing best_model.pt (and optionally metadata.json), or stage one with scripts/mlops/stage_serving_model.sh."
        )
...
def predict(req: PredictionRequest):
        ...
        if e.src in node_id_map and e.dst in node_id_map:
            ...
            for s, d in [(si, di), (di, si)]:
                ...
                edge_attr_list.append(
                    [e.distance, e.rssi, e.snr, e.delay, e.packet_loss, e.relative_speed, e.throughput]
                )
    ...
    for i, (src, dst) in enumerate(query_pairs):
        if src in node_id_map and dst in node_id_map:
            ...
            predictions.append(
                EdgePrediction(
                    src=src, dst=dst, stability_score=score, stable=score >= _threshold, routing_weight=1.0 - score
                )
            )
